/**
 * WhatsApp XMD Bot - Main Entry Point
 */
...
async function startBot() {
  ...
  // Check if sessionID is provided and process LunaBot! format session
  if (config.sessionID && config.sessionID.startsWith('LunaBot!')) {
    try {
      const [header, b64data] = config.sessionID.split('!');
     
      if (header !== 'LunaBot' || !b64data) {
        throw new Error("❌ Invalid session format. Expected 'LunaBot!.....'");
      }
      ...
      console.log('📡 Session : 🔑 Retrieved from LunaBot Session');
     
    } catch (e) {
      console.error('📡 Session : ❌ Error processing LunaBot session:', e.message);
...
// Start the bot
console.log('🚀 Starting WhatsApp XMD Bot...\n');
console.log(`📦 Bot Name: ${config.botName}`);
console.log(`⚡ Prefix: ${config.prefix}`);
const ownerNames = Array.isArray(config.ownerName) ? config.ownerName.join(',') : config.ownerName;
console.log(`👑 Owner: ${ownerNames}\n`);
